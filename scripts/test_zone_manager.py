#!/usr/bin/env python
"""Manual smoke-test for ZoneManager, chained onto the full pipeline so far.

Usage:
    python scripts/test_zone_manager.py                                   # use .env / config.yaml / config/zones.yaml
    python scripts/test_zone_manager.py --source-type file --source data/input/sample.mp4
    python scripts/test_zone_manager.py --demo                            # self-contained, no video file needed
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Smoke-test the full VideoInput -> ... -> ZoneManager chain.")
    parser.add_argument(
        "--source-type",
    ...
    parser.add_argument("--zones", default=DEFAULT_ZONES_CONFIG_PATH, help="Path to zones.yaml")
    parser.add_argument(
        "--demo",
        action="store_true",
...
def build_demo_video() -> str:
    ...
    output_path = Path("data/input/_demo_zone_manager.mp4")
        ...
        with VideoInput(video_config) as video_input:
            ...
            for memberships in zone_manager.evaluate_stream(tracks_stream):
                for membership in memberships:
                    status = "INSIDE" if membership.inside else "outside"
                    print(
                        f"frame={membership.frame_index} track={membership.track_id} "
                        f"zone={membership.zone_id:<20} {status}"
                    )
