#!/usr/bin/env python
"""Manual smoke-test for LoiteringDetector, chained onto the full pipeline so far.

From ZoneManager onward the pipeline is a fan-out/fan-in, not a simple
linear chain: LoiteringDetector needs both `tracks` and the `memberships`
derived from them for the same frame, and IntrusionDetector must run
before LoiteringDetector each frame (it's what refreshes
first_seen_inside_timestamp). So this script uses a manual per-frame
loop instead of nested generator chaining for this final stage.

Usage:
    python scripts/test_loitering_detection.py                                   # use .env / config.yaml / config/zones.yaml
    python scripts/test_loitering_detection.py --source-type file --source data/input/sample.mp4
    python scripts/test_loitering_detection.py --demo                            # self-contained, low threshold for a quick demo
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Smoke-test the full VideoInput -> ... -> LoiteringDetector chain.")
    parser.add_argument(
        "--source-type",
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Override loitering.threshold_seconds (defaults to config.yaml, or 0.05s under --demo for a quick result)",
    )
    parser.add_argument(
        "--demo",
        action="store_true",
        help="Build a self-contained looped clip from ultralytics' bundled zidane.jpg, with a low "
        "threshold so a loitering event fires within a handful of frames.",
    )
...
def build_demo_video() -> str:
    ...
    output_path = Path("data/input/_demo_loitering.mp4")
...
def build_demo_zones() -> str:
    output_path = Path("data/input/_demo_loitering_zones.yaml")
...
def main() -> int:
    ...
    threshold_seconds = args.threshold_seconds if args.threshold_seconds is not None else (0.05 if args.demo else None)
    loitering_config = LoiteringConfig(threshold_seconds=threshold_seconds) if threshold_seconds is not None else None
    ...
    try:
        with VideoInput(video_config) as video_input:
            ...
            for tracks in tracker.track_stream(detections_stream):
                ...
                intrusion_detector.evaluate(memberships)  # must run first
                for event in loitering_detector.evaluate(tracks, memberships):
                    print(
                        f"frame={event.frame_index:<4} LOITERING track={event.track_id} "
                        f"zone={event.zone_name} dwell={event.dwell_time_seconds:.2f}s "
                        f"(threshold={event.threshold_seconds:.2f}s, event_id={event.event_id})"
                    )
