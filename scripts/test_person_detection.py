#!/usr/bin/env python
"""Manual smoke-test for PersonDetector, chained onto VideoInput -> FrameProcessor.

Usage:
    python scripts/test_person_detection.py                                   # use .env / config.yaml
    python scripts/test_person_detection.py --source-type file --source data/input/sample.mp4
    python scripts/test_person_detection.py --demo                            # self-contained, no video file needed
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Smoke-test VideoInput -> FrameProcessor -> PersonDetector.")
    parser.add_argument(
        "--source-type",
...
def build_demo_video() -> str:
    ...
    output_path = Path("data/input/_demo_person_detection.mp4")
...
def main() -> int:
    ...
    print(
        f"Detector: model={detector_config.model_path}, device={detector_config.device}, "
        f"conf>={detector_config.confidence_threshold}"
    )
        ...
        with VideoInput(video_config) as video_input:
            ...
            for detections in detector.detect_stream(processed_frames):
                ...
                for detection in detections:
                    ...
                    print(
                        f"frame={detection.frame_index} conf={detection.confidence:.2f} "
    ...
    print(f"Done. Source frames: {frame_count}, total person detections: {total_detections}")
    return 0
