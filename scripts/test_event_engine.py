#!/usr/bin/env python
"""Manual smoke-test for EventEngine, chained onto the full pipeline so far.

Usage:
    python scripts/test_event_engine.py                                   # use .env / config.yaml / config/zones.yaml
    python scripts/test_event_engine.py --source-type file --source data/input/sample.mp4
    python scripts/test_event_engine.py --demo                            # self-contained, low threshold for a quick demo
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Smoke-test the full VideoInput -> ... -> EventEngine chain.")
    parser.add_argument(
        "--source-type",
...
def build_demo_video() -> str:
    ...
    output_path = Path("data/input/_demo_event_engine.mp4")
...
def build_demo_zones() -> str:
    output_path = Path("data/input/_demo_event_engine_zones.yaml")
    ...
    try:
        with VideoInput(video_config) as video_input:
            ...
            for tracks in tracker.track_stream(detections_stream):
                ...
                for event in event_engine.evaluate(intrusion_events, loitering_events):
                    print(
                        f"frame={event.frame_index:<4} [{event.severity.value:<6}] {event.event_type.value:<16} "
                        f"track={event.track_id} zone={event.zone_name} source={event.source.value} "
                        f"payload={dict(event.payload)} (event_id={event.event_id})"
                    )
